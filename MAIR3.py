...
def main():
    ...
    welcome_msg = 'system: Hello, welcome to the greatest MAIR restaurant system! You can ask for restaurants by area, price range or food type. How may I help you?'
    ...
    while dialog.terminate == 0:
        # Dialog with the user starts by asking input and transforming it to be used by the extractor
        utterance = input('user: ').replace('?', '').replace('!', '').replace('.', '').replace(',', '').lower() 
        # Extractor extracts information from the user utterance and stores them in the dialog class.
        dialog.extractor(utterance)
        # The response is generated by the responder function based on the information stored in the dialog class.
